...
def pivot_months_pandas(data):
    ...
    """
    data['month'] = data['date'].dt.strftime('%Y-%m-%d').str[:7]
    grouped_data = data.groupby(['name','month'])
    percip_sum = grouped_data.aggregate('sum').reset_index()
    monthly = percip_sum.pivot(index = 'name', columns= 'month', values = 'precipitation')
    percip_count = grouped_data.aggregate('count').reset_index()
    counts = percip_count.pivot(index = 'name', columns= 'month', values = 'precipitation')
